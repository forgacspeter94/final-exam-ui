...
const bookCard = (number, title, sub, text) => {
    return`
    <div class='card'>
    ...
    <p class='text'>${text}</p>
    <button>read more  <span id='arrow' class="material-icons">arrow_forward</span></button>
	</div>
    `
}

const createCard = (books) => {
    return books.cards.map((book,index) => bookCard(index+1, book.title, book.sub, book.text)).join(' ')
}
...
const loadEvent = () => {
    ...
    rootElement.insertAdjacentHTML('beforeend', container)

    const bookCardsDivElement = document.getElementById('container')
    bookCardsDivElement.insertAdjacentHTML('beforeend', createCard(books))
